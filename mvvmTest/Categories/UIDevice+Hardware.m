...
{
    NSString *uid = nil;
    uid = [SFHFKeychainUtils getPasswordForUsername:[NSBundle mainBundle].bundleIdentifier andServiceName:[NSBundle mainBundle].bundleIdentifier error:nil];
    if (uid)
        return uid;

    if ([UIDevice currentDevice].systemVersion.intValue>= 7)
        uid = [[NSString UUIDStringWithMD5] lowercaseString];
    else
        uid = [self macaddress];

    assert(uid != nil);
    [SFHFKeychainUtils storeUsername:[NSBundle mainBundle].bundleIdentifier andPassword:uid forServiceName:[NSBundle mainBundle].bundleIdentifier updateExisting:YES error:nil];
    return uid;
}
...
-(NSString *)machineName
{
    ...
    NSString *machineKey = [NSString stringWithCString:systemInfo.machine encoding:NSUTF8StringEncoding];
    
    if ([machineKey length]>0 ) {
        NSString *deviceName=[HARDWARE objectForKey:machineKey];
